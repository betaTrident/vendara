...
description: >-
  Analyze raw prompts, identify intent and gaps, match workspace components
...
Analyze a draft prompt, critique it, match it to workspace components,
...
- User says "how should I use the workspace for..."
...
- User is asking about workspace configuration (use the matching configuration workflow instead)
...
- "Optimize this prompt for the workspace"
- "Rewrite this prompt so the agent uses the right commands"
- "帮我优化这个指令"
- "How should I prompt the workspace for this task?"
...
|-----------|------------------|
| `configure-workspace` | User hasn't set up the workspace yet |
